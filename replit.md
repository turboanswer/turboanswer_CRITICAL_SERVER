...
### Key Features
- **Multi-Model AI System**: 4 tiers - Free (Gemini Flash Lite), Pro ($6.99/mo - Gemini Flash), Research ($30/mo - Claude Opus 4.5 primary + Gemini 3.1 Pro fallback + Code Studio + AI Video Studio), Enterprise ($100/mo - Research access + shareable 6-digit code for up to 5 team members, 44% savings vs individual Research plans). Research/Enterprise complex queries use Claude as primary with Gemini fallback. Research tier includes Pro access. Enterprise includes Research access. Custom team plans available via support@turboanswer.it.com for teams larger than 5.
- **Smart Model Routing**: Complexity classifier in `server/services/multi-ai.ts` routes Research/Enterprise queries: simple queries (short, factual, greetings, <220 chars without technical signals) → Gemini Flash Lite (2000 tokens, temp 0.4); complex queries (code blocks, technical keywords like implement/algorithm/architecture/optimize/debug, multi-step, >220 chars) → Claude Opus 4.5 primary (8192 tokens, temp 0.1) with Gemini 3.1 Pro fallback. Free/Pro tiers always use Gemini Flash Lite. This saves cost without degrading quality since Flash handles simple questions equally well.
...
- **Code Studio** (`/code-studio`): Full in-browser IDE — **$15/month add-on** with **7-day free trial** ($1.00 trial budget, $0 for 7 days, then $15/month). Available to any subscription tier. **Complexity-based pricing: Micro $0.10, Simple $0.35, Standard $0.75, Complex $1.50, Advanced $3.00.** AI classifies prompt complexity BEFORE building — user sees the cost and confirms before credit is deducted. Monthly budget: $15.00 (1500 cents) resets each billing cycle — unused budget does NOT roll over. Zero balance = blocked. Estimate endpoint: `POST /api/code/estimate-complexity`. When budget runs out, users are prompted to buy more. Budget packs: $5/500¢, $10/1000¢, $25/2500¢, $50/5000¢, $100/10000¢ — one-time PayPal Orders v2 payments, pack budget never expires. Supports promo code entry on the paywall — 100% off codes activate access for free (no PayPal). Backend route: `POST /api/apply-code-studio-promo`. Owner lifetime code seeded on startup: `TURBOCODE-FOREVER`. Monaco editor (VS Code engine) with multi-file support, file tabs, syntax highlighting for 8+ languages. Live preview served from real URL `/code-preview/:id` (no sandbox restrictions) after build; srcDoc used only for live editing. Code execution via Piston API. Gemini 3.1 Pro AI sidebar. Save projects to PostgreSQL DB. Deploy to a public URL at `/p/:slug`. Backend gate: all `/api/code/*` routes require `codeStudioAddon === true`. Budget reset logic: `codeStudioCreditsResetAt` stores the NEXT reset date; when that date passes, credits reset to exactly 1500 cents (no rollover) and next reset = now + 30 days. On trial signup: 100 cents ($1.00), nextReset = now + 7 days. Credit routes: `GET /api/code/credits`, `POST /api/code/buy-credits`, `POST /api/code/capture-credits`. Schema: `codeStudioAddon`, `codeStudioAddonSubId`, `codeStudioCredits` (integer = cents), `codeStudioCreditsResetAt` (timestamp = next reset date). Header shows a budget bar (% of $15 used) with dollar display. Enterprise subscribers: Code Studio included FREE — auto-activated with subId `enterprise_included`.
