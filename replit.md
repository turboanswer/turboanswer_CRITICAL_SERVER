...
- **Subscription Management**: PayPal Subscriptions API integration for Pro ($6.99/month), Research ($25/month), and Enterprise ($70/month) plans. All paid plans include a 7-day free trial (no charge on signup day). Payment popups trigger from model selector. Uses PayPal REST API with auto-created billing plans. Plans auto-recreate with trial period on startup if missing. Previous plan IDs (old prices): Pro P-5T439024E40002735NGU42YY, Research P-3J114811P66183426NGU42YY (was $15), Enterprise P-8MA89259B73604813NGU42YY (was $50) — new plans are auto-created on server restart with correct prices. Current active plan IDs: Pro P-5T439024E40002735NGU42YY, Research P-9P801014WA200020LNGWHFZI ($25), Enterprise P-4TP76281350076834NGWHFZI ($70). Enterprise subscribers get a secure 6-digit numeric code saved to their account (persists across sessions) to share with up to 5 team members for Research access. Custom pricing available for larger teams via support email.
...
- **Photo Editor** (`/photo-editor`): Enterprise-exclusive AI photo studio powered by Google Imagen 3. Three modes: (1) Generate — text-to-image via Imagen 3 REST API with aspect ratio control and style presets; (2) Edit — upload any photo and describe changes (remove people, swap backgrounds, add objects) via Gemini 2.0 Flash multimodal image-in/image-out; (3) Place Me — upload a selfie and describe a scene, AI composites you there realistically. Backend routes: `POST /api/photo-editor/generate` (Imagen 3) and `POST /api/photo-editor/edit` (Gemini multimodal).
- **Music Studio** (`/music-studio`): Enterprise-exclusive Lyria Music Studio. User enters song idea, genre, and mood; Gemini generates complete song JSON (title, BPM, key, time signature, chord progression, full section lyrics for verse/pre-chorus/chorus/bridge/outro). Web Audio API synthesizer plays chords and melody using oscillators with ADSR envelopes. Real-time section highlighting during playback. Download song as plain-text file. Backend route: `POST /api/music/generate`.
...
- **Code Studio**: Full in-browser IDE at `/code-studio`. Monaco editor (VS Code engine) with multi-file support, file tabs, syntax highlighting for 8+ languages. Live preview iframe for HTML/CSS/JS apps with auto-refresh. Code execution for Python, JavaScript, TypeScript, Java, C++, Rust, Go, etc. via Piston API (free, no key needed). Gemini 3.1 Pro AI sidebar for generating, debugging, and explaining code. Save projects to PostgreSQL DB. Deploy to a public shareable URL at `/p/:slug`. Custom domain field with DNS setup instructions. Accessible from chat header via "Code" button. No subscription tier restriction.
