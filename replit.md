...
- **Photo Editor** (`/photo-editor`): Free for all users. AI photo studio. Three modes: (1) Generate — text-to-image with aspect ratio control and style presets, powered by `gemini-2.0-flash-exp-image-generation` via `@google/genai` SDK (typically 3–10s, ~1MB PNG output); (2) Edit — upload any photo and describe changes (remove people, swap backgrounds, add objects), powered by gpt-image-1; (3) Place Me — upload a selfie and describe a scene, AI composites you there realistically, powered by gpt-image-1. Backend routes: `POST /api/photo-editor/generate` (Gemini) and `POST /api/photo-editor/edit` (gpt-image-1 via Replit integration).
