...
- **Deep Research Mode**: Available for Research/Enterprise tier users. Uses direct Gemini 3.1 Pro (`gemini-3.1-pro-preview`) via generateContent API with a research-depth system prompt (thorough structure, code with explanations, 8192 max tokens, temp 0.1). Synchronous — returns immediately, no polling. Toggle button in chat header turns violet when active. Shows "Gemini 3.1 Pro is thinking deeply…" progress bubble. Messages stored in conversation on completion.
